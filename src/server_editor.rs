...
use druid::{
    commands, lens::Identity, widget::{Button, Flex, Label, LineBreaking, List, ListIter, Scroll, Switch}, Data, Lens, LensExt, Widget, WidgetExt
};

use livesplit_core::settings::{SettingsDescription, Value};

use crate::{
    config,
    consts::{switch_style, BUTTON_SPACING, DIALOG_BUTTON_HEIGHT, DIALOG_BUTTON_WIDTH, GRID_BORDER, MARGIN}, settings_table::{self, SettingsRow},
};
...
pub struct State {
    ...
    #[data(ignore)]
    pub editor: Rc<RefCell<Option<config::Server>>>,
    #[data(ignore)]
    pub closed_with_ok: bool,
}

impl State {
    pub(crate) fn new(editor: config::Server) -> Self {
        Self {
